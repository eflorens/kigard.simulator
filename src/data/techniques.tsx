import { Bold, Italic, List, Row } from "../components";
...
import { BoxType, DamageType, Talent } from "./talents";
...
const addModifier = (value?: number, modifier?: number) =>
  (value && (value + (modifier ?? 0))) || value;

const getModifierLabel = (modifier?: number) =>
  (modifier && ` ${(modifier > 0 && " + ") || " - "} ${modifier} PA`) || "";


const getPrimaryWeaponUsageCost = (summary: SummaryState, modifier?: number) => {
  return addModifier(summary.primaryWeapon?.usageCost, modifier) ?? `(Coût Arme${getModifierLabel(modifier)})`;
}
...
function WeaponAttackResult({ weapon, modifier }: Readonly<{ weapon?: LightWeapon, modifier?: WeaponModifier }>) {
  ...
  return (
    <span>
      {isWeapon && (
        <>
          <Bold>PRE : {addModifier(Math.floor(baseAccuracy) + weapon.accuracy, modifier?.accuracy)}%</Bold>
          <span> et </span>
          <Bold>DGT : {addModifier(Math.floor(baseDamage) + weapon.damage, modifier?.damage)}</Bold>
          <Bold> [Bloqué : {addModifier(Math.floor(baseDamage / 2) + weapon.damage, modifier?.damage)}]</Bold>
          <Bold> [Critique : {addModifier(Math.floor(criticalBaseDamage) + weapon.damage, modifier?.damage)}]</Bold>
          {weapon.status?.map(({ value, status }) => (
            <span key={status}>
              <span> et </span>
              <Bold>{value}<DisplayStatus status={status} /></Bold>
            </span>
          ))}
        </>
      )}
...
function PrincipalWeaponAttack({ summary, modifier }: Readonly<{ summary: SummaryState, modifier?: WeaponModifier }>) {
  return (
    <span>
      ...
      <WeaponAttackResult weapon={summary?.primaryWeapon} modifier={modifier} />
    </span>
  );
}

// Generate collection of Talent from TechniqueId
export const techniques: Talent[] = [
  {
    id: TechniqueId.Stun,
    name: "Assomé",
        ...
        <>
          ...
          <Row>
            <span>
              ...
              <WeaponAttackResult weapon={summary?.primaryWeapon} />
            </span>
          </Row>
          <Row>
    ...
    getDescription: (summary) => {
      ...
      return (
        <span>
          ...
          <WeaponAttackResult weapon={summary?.primaryWeapon} modifier={{ baseDamage }} />
        </span>
      )
    },
  }, {
    ...
    getDescription: (summary) => (
      <span>
        ...
        <WeaponAttackResult weapon={summary?.secondaryWeapon} modifier={{ baseDamage: summary.constitution, forceWeapon: true }} />
      </span>
    ),
  }, {
      ...
      <>
        <Row>
          <span>
            ...
            <WeaponAttackResult weapon={summary?.primaryWeapon} modifier={{ accuracy: -15 }} />
          </span>
        </Row>
        <Row>
    ...
    getDescription: (summary) => (
      <>
        ...
        <Row>
          <span>Confère également <Bold>{summary.constitution / 5} <DisplayStatus status={Status.Impact} hasLabel /></Bold> sur la case ciblée</span>
        </Row>
        <Row>
    ...
    getDescription: () => (
      <>
        ...
        <Row>
          <span>
            <span>Un <Bold>fumigène</Bold> peut être utilisé pour obtenir <Bold>+30 en DIS</Bold> pendant l'action</span>
          </span>
        </Row>
      </>
    ),
        ...
        <>
          ...
          <Row>
            <span>
              ...
              <WeaponAttackResult weapon={noWeapon} modifier={{ baseDamage: summary.dexterity, baseAccuracy: summary.discretion }} />
            </span>
          </Row>
          <Row>
          ...
          <Row>
            <span>Les objets servants à poser des pièges sont <Bold>Flèche</Bold> et <Bold>Piège à ours</Bold></span>
          </Row>
        </>
      )
    },
    ...
    getDescription: (summary) => (
      <>
        <Row>
          <span>Réalise un <Bold>soutien</Bold> qui confère <Bold>{summary.charisma / 5}</Bold> au lieu de 2</span>
        </Row>
        <Row>
      ...
      <>
        <Row>
          <span>
            ...
            <span>vous gagnez <Bold>+[5 dégâts inflgés/5] <DisplayStatus status={Status.Overload} /></Bold></span>
          </span>
        </Row>
        <Row>
          <span>
            <span>Quand vous infligez des <Bold>dégâts magiques, </Bold></span>
            <span>vous gagnez <Bold>+[5 dégâts inflgés/5] <DisplayStatus status={Status.Impact} /></Bold></span>
          </span>
        </Row>
        <Row>
          <span>
